...
#   None
# Returns:
#   0 if all required commands exist, exits with error otherwise
#######################################
_valid_required() {
...
# 执行安装前的环境验证
# Globals:
#   CLASH_BASE_DIR - 安装目录路径
#   ZSH_VERSION - zsh 版本号（如果使用 zsh）
#   BASH_VERSION - bash 版本号（如果使用 bash）
# Arguments:
#   None
# Returns:
#   0 if validation passes, exits with error otherwise
#######################################
_valid() {
    # 验证必需命令
    _valid_required
...
# 准备安装所需的压缩包文件
# Globals:
#   KERNEL_NAME - 内核名称（clash 或 mihomo）
#   ZIP_CLASH, ZIP_MIHOMO, ZIP_YQ, ZIP_SUBCONVERTER, ZIP_UI - 各组件压缩包路径
#   BIN_BASE_DIR - 二进制文件基础目录
# Arguments:
#   None
# Returns:
#   0 on success, exits with error if download or extraction fails
#######################################
_prepare_zip() {
    # 加载已存在的压缩包路径
    _load_zip >&/dev/null
...
# 加载压缩包文件路径
# Globals:
#   ZIP_BASE_DIR - 压缩包存放目录
#   ZIP_CLASH, ZIP_MIHOMO, ZIP_YQ, ZIP_SUBCONVERTER, ZIP_UI - 设置各组件压缩包路径
# Arguments:
#   None
# Returns:
#   0 on success
...
_load_zip() {
    ...
    # UI 压缩包如果未设置则尝试加载（支持多种格式）
    if [ -z "$ZIP_UI" ]; then
        for pattern in dist.zip compressed-dist.tgz dist.tar.gz; do
            local found="${ZIP_BASE_DIR}/$pattern"
            [ -f "$found" ] && ZIP_UI="$found" && break
        done
    fi

    # 打印加载的压缩包路径
    _okcat '📦' "压缩包配置："
    _okcat '  ' "ZIP_CLASH: $ZIP_CLASH"
    _okcat '  ' "ZIP_MIHOMO: $ZIP_MIHOMO"
    _okcat '  ' "ZIP_YQ: $ZIP_YQ"
    _okcat '  ' "ZIP_SUBCONVERTER: $ZIP_SUBCONVERTER"
    _okcat '  ' "ZIP_UI: $ZIP_UI"
}
...
_download_zip() {
    ...
    *)
    ...
    local -A urls=(
        ...
        [ui]="https://github.com/metacubex/metacubexd/releases/latest/download/compressed-dist.tgz"
    )
...
#   $@ - 需要验证的压缩包文件路径列表
# Returns:
#   0 if all files are valid, exits with error if any file is invalid
#######################################
_valid_zip() {
    (($#)) || return 1
    local zip fail_zips=()
...
# 解压所有必需的压缩包文件
# Globals:
#   ZIP_KERNEL, ZIP_YQ, ZIP_SUBCONVERTER, ZIP_UI - 各组件压缩包路径
#   BIN_KERNEL, BIN_BASE_DIR, BIN_SUBCONVERTER_DIR, BIN_SUBCONVERTER_CONFIG - 二进制文件路径
#   RESOURCES_BASE_DIR - 资源文件目录
# Arguments:
#   None
# Returns:
#   0 on success, exits with error if validation or extraction fails
#######################################
_unzip_zip() {
    # 验证所有压缩包文件
    ...
    # 解压 UI 文件到 dist 目录（支持 zip 或 tar 格式）
    local ui_dir="${RESOURCES_BASE_DIR}/dist"
    mkdir -p "$ui_dir"
    unzip -oqq "$ZIP_UI" -d "$ui_dir" 2>/dev/null || tar -xf "$ZIP_UI" -C "$ui_dir"
